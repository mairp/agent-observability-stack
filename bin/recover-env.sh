#!/usr/bin/env bash
# One-shot recovery: rebuild .env from the running hypervisor-exporter container's baked-in
# environment + LITELLM_MASTER_KEY from ~/litellm/.env. Never prints secret values.
set -euo pipefail
cd "$(dirname "$0")/.."

CTR=observability-hypervisor-exporter-1
OUT=.env
TMP="$(mktemp)"
trap 'rm -f "$TMP"' EXIT
...
umask 077
{
  ...
  grep -m1 '^LITELLM_MASTER_KEY=' ~/litellm/.env || echo 'LITELLM_MASTER_KEY='
} > "$OUT"
chmod 600 "$OUT"
